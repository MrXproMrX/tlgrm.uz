   ...
   <section>
      <ul class="faq_stickers__menu">
         <li v-for="(faqSticker, i) in faqStickers" :key="i">
            <a
               class="faq_stickers__menu__link"
               :href="`#mrx` + faqSticker.id"
               >{{ faqSticker.title }}</a
            >
         </li>
      </ul>
      ...
      <div class="faq_stickers__list">
         <div
            class="faq_stickers__item"
            v-for="faqSticker in faqStickers"
            :key="faqSticker.id"
         >
            <h3
               class="faq_stickers__title__h3"
               v-if="faqSticker.title"
               :id="`mrx` + faqSticker.id"
            >
               {{ faqSticker.title }}
            </h3>
            <div
               class="faq_stickers__text"
               v-html="faqSticker.text"
               v-if="faqSticker.text"
            ></div>
            ...
            <div class="faq__delete__menu" v-if="faqSticker.itemDelete">
               <div
                  class="faq__delete__item"
                  v-for="(items, i) in faqSticker.itemDelete"
                  :key="i"
               >
                  <div
                     class="faq_stickers__text"
            ...
            <div class="faq__error" v-if="faqSticker.errors">
               <div
                  class="faq__error__item"
                  v-for="(items, i) in faqSticker.errors"
                  :key="i"
               >
                  <h3 class="faq__error__title faq_stickers__title__h3">
                     {{ items.title }}
                  </h3>
            ...
            <div class="faq__verification" v-if="faqSticker.itemImgs">
               <div
                  class="faq__verification__img"
                  v-for="(items, i) in faqSticker.itemImgs"
                  :key="i"
               >
