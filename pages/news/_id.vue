                     ...
                     <section v-if="NewsBlog.listItem">
                        <ul class="newsIn__bootom__list">
                           <li v-for="(item, i) in NewsBlog.listItem" :key="i">
                              <h3 class="newsIn__title__h3" v-if="item.title">
                                 {{ item.title }}
                              </h3>
                              <div
                                 class="newsIn__bootom__text"
                                 v-if="item.text"
                                 v-html="item.text"
                              ></div>
...
export default {
   ...
   computed: {
      NewsBlog() {
         return newsListApi.find(
            (NewsBlog) => NewsBlog.id === +this.$route.params.id
         );
      },
   },
};
